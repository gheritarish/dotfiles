...
#+begin_src emacs-lisp
  ...
  (setq read-process-output-max (* 1024 1024)) ;; 1MB (increased from 512kb)

  ;; LSP file watchers - optimize for performance
  (setq lsp-enable-file-watchers nil)      ; Disable file watchers - major performance improvement
#+end_src
...
#+begin_src emacs-lisp
  (use-package emojify
    :demand t
    :config
    ;; Set emoji fallback fonts for proper emoji rendering
    ;; These fonts will be used when the main font doesn't have emoji glyphs
    (set-fontset-font t 'symbol "Noto Color Emoji" nil 'prepend)
    (set-fontset-font t 'emoji "Noto Color Emoji" nil 'prepend)
    
    ;; Configure emojify to use unicode display and support both unicode and github styles
...
On-demand spellchecking for French and English in org-mode documents.

#+begin_src emacs-lisp
;; Configure ispell to use aspell
(setq ispell-program-name "aspell"
      ispell-extra-args '("--sug-mode=ultra" "--lang=en_US"))

;; Function to toggle flyspell mode with French dictionary
(defun my/flyspell-french ()
  "Enable flyspell-mode with French dictionary."
  (interactive)
  (ispell-change-dictionary "francais")
  (flyspell-mode 1)
  (flyspell-buffer)
  (message "Flyspell enabled with French dictionary"))

;; Function to toggle flyspell mode with English dictionary
(defun my/flyspell-english ()
  "Enable flyspell-mode with English dictionary."
  (interactive)
  (ispell-change-dictionary "en_US")
  (flyspell-mode 1)
  (flyspell-buffer)
  (message "Flyspell enabled with English dictionary"))

;; Function to disable flyspell
(defun my/flyspell-off ()
  "Disable flyspell-mode and clear highlights."
  (interactive)
  (flyspell-mode -1)
  (message "Flyspell disabled"))

;; Customize flyspell faces to make errors more visible
(custom-set-faces
 '(flyspell-incorrect ((t (:underline (:color "#f38ba8" :style wave)))))
 '(flyspell-duplicate ((t (:underline (:color "#f9e2af" :style wave))))))
#+end_src
...
(use-package lsp-mode
  ...
  (setq lsp-signature-auto-activate nil)     ; Disable auto signature help
  ...
  (setq lsp-ui-doc-enable nil              ; Disable doc popup on cursor - major performance drain
        lsp-ui-doc-show-with-cursor nil
        lsp-ui-doc-position 'at-point
        lsp-ui-sideline-enable t
        lsp-ui-sideline-show-diagnostics t
        lsp-ui-sideline-delay 0.5))        ; Add delay to reduce constant updates
...
Ensure I have LSP in ~python~. Using only LSP-mode (not Eglot) to avoid conflicts.

#+begin_src emacs-lisp
(use-package python
  :straight nil
  :config
  ;; Smart backspace for Python indentation
  (defun my/python-smart-backspace ()
    "Delete back to previous indentation level in Python, or delete one char."
    (interactive)
    (if (and (looking-back "^[[:space:]]+" (line-beginning-position))
             (not (bolp)))
        ;; At beginning of line with only whitespace - delete to previous indent level
        (let* ((current-indent (current-indentation))
               (target-indent (max 0 (- current-indent python-indent-offset))))
          (delete-region (line-beginning-position)
                         (+ (line-beginning-position) current-indent))
          (insert (make-string target-indent ?\s)))
      ;; Otherwise, use the normal electric-pair delete behavior
      (call-interactively 'my/electric-pair-delete-pair)))
  
  ;; Bind smart backspace in Python mode for both Evil and Emacs
  (add-hook 'python-mode-hook
            (lambda ()
              (define-key evil-insert-state-local-map (kbd "DEL") 'my/python-smart-backspace)))
...
Typeset LaTeX equations inline without exporting. Org renders a fragment (an
inline ~$...$~ or a ~\begin{equation}~ block) into an SVG of the typeset math;
[[https://github.com/io12/org-fragtog][org-fragtog]] auto-toggles it: the equation typesets when the cursor leaves it and
reverts to editable LaTeX when you enter it.

Uses ~dvisvgm~ for crisp, scalable output (both ~latex~ and ~dvisvgm~ are on PATH).
...
#+begin_src emacs-lisp
  ...
  (use-package ox-rst
    :after org
    :demand t  ; override use-package-always-defer — ox-rst must load eagerly after org
    :custom
    ;; Use Sphinx's code-block directive (supports :emphasize-lines:, :lineno-start:, etc.)
    (org-rst-code-block 'code-block)
    :config
    (require 'ox-rst)
    ;; Teach ox-rst to emit :emphasize-lines: from #+ATTR_RST: :emphasize-lines N,M
    (defun my/org-rst-src-block (src-block _contents info)
      "Export SRC-BLOCK to RST, supporting :emphasize-lines: from #+ATTR_RST:."
      (when (org-string-nw-p (org-element-property :value src-block))
        (let* ((lang      (org-element-property :language src-block))
               ...
               (attrs     (org-export-read-attribute :attr_rst src-block))
               (class     (plist-get attrs :class))
               (emphasize (plist-get attrs :emphasize-lines))
               (lst-lang  (or (cadr (assq (intern (or lang ""))
                                          org-rst-pygments-langs))
                              lang)))
          (concat
           (format ".. code-block:: %s\n" lst-lang)
           (when num-start
             (format "    :lineno-start: %s\n" (1+ num-start)))
           (when emphasize
             (format "    :emphasize-lines: %s\n" emphasize))
           (when class
             (format "    :class: %s\n" class))
           (when label
             (format "    :name: %s\n" label))
    ...
    (advice-add 'org-rst-src-block :override #'my/org-rst-src-block))
